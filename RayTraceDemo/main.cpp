...
void colorCalc(int startValue, int endValue, int image_width, int samples_per_pixel, int max_depth, ::hittable_list world, ::vec3
	background, std::chrono::steady_clock::time_point begin, const int image_height, camera cam,
	int total, shared_ptr<hittable_list> lights)
{
	/*auto theseLights = make_shared<hittable_list>();
	theseLights->add(lights);*/
...
void threadRun(int image_width, int samples_per_pixel, int max_depth, hittable_list world, color background,
	std::chrono::steady_clock::time_point begin, const int image_height, RGB* data, int index, camera cam,
	int total, shared_ptr<hittable_list> lights)
{
...
int main()
{
	...
	lights->add(std::make_shared<xz_rect>(213, 343, 227, 332, 554, std::make_shared<material>()));
	lights->add(std::make_shared<sphere>(point3{ 190, 90, 190 }, 90, std::make_shared<material>()));
